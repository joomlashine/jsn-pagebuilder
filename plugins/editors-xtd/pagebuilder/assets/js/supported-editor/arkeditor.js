...
(function ($) {
    $.JSNPbEditorHelper = function () {
        ...
        JSNPbEditorHelper.initEntryButtons = function (source_id, pbEntryButtons) {
            ...
            var container = sourceId.parent();
            var buttonGroups = container.prepend(pbEntryButtons);

            var editor = $('<div class="editor">');
            sourceId.before(editor).appendTo(editor);

            var wrapperOverflow = sourceId.parents('.editor').css('overflow');
            ...
            $('.switchmode-button.pb-on', buttonGroups).on('click', function () {
                if (!$(this).hasClass('active')) {
                    ...
                    sourceId.parents('.editor').css('overflow', 'inherit');
                }
            });
            ...
            $('.switchmode-button.pb-off', buttonGroups).on('click', function () {
                if (!$(this).hasClass('active')) {
                    ...
                    sourceId.parents('.editor').css('overflow', wrapperOverflow);
                    $('#' + source_id + ' #cke_jform_content', container).addClass('editor-hidden').hide();
         ...
         */
            JSNPbEditorHelper.hideEditor = function (container, source_id) {
				...
				$('#' + source_id + ', #cke_text, #editor-xtd-buttons', container).addClass('editor-hidden').hide();
